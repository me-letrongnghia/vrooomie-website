...
export class DeliveryAddressModalComponent implements OnInit, OnDestroy {
  ...
  loadGoogleMaps(): void {
    ...
    if (MAPS_CONFIG.API_KEY === 'YOUR_GOOGLE_MAPS_API_KEY') {
      console.warn('Google Maps API key not configured. Map will not be displayed.');
      this.mapLoaded = false;
      // Don't show error, just don't load the map
      return;
    }
  ...
  onAddressChange(event: any): void {
    ...
    // Get address suggestions (only if Google Maps is loaded)
  ...
  calculateDelivery(): void {
    ...
    // If Google Maps is not loaded, use simplified calculation
    if (!this.mapLoaded) {
      setTimeout(() => {
        // Use a simplified distance calculation (estimate based on address length/complexity)
        // In production, you would call a backend API for accurate geocoding
        const estimatedDistance = 5; // Default 5km for demo
        const deliveryFee = this.BASE_DELIVERY_FEE + (estimatedDistance * this.DELIVERY_RATE);

        this.selectedAddress = this.deliveryForm.get('address')?.value;
        this.calculatedDistance = estimatedDistance;
    ...
    // Original Google Maps-based calculation
    setTimeout(() => {
